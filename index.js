...
function nextNewYear() {
  const today = new Date(); // Створюєму змінну в яку записуємо поточну дату
  const year = today.getFullYear(); // Визначення поточного року.
  const nextYear = new Date(year + 1, 0, 1); // Визначення дати наступного Нового року. Для цього створюємо новий об`єкт Date в якому збільшуємо поточний рік на 1, встановлюємо місяць на 0 а дату на 1
  const diff = nextYear - today; // Визначаємо кількість мілісекунд до наступного Нового року в змінну diff.
  // Розраховуємо кількість повних днів, годин, хвилин і секунд.
  const days = diff / (1000 * 60 * 60 * 24); // Дні:diff / (1000 * 60 * 60 * 24)
  ...
  const miliseconds = diff % 1000; // Мілісекунди:  diff % 1000;
...
function isLeapYear(year) {
  if (typeof year !== "number") {
    ...
    return "Ви не ввели рік";
  } else {
    return year % 4 === 0
      ? { year: year, isLeap: true }
      : { year: year, isLeap: false };
...
function getDayOfWeek(date) {
  if (typeof date.getTime !== "function") {
    ...
    return { inputDate: dateString, dayOfWeek: dayOfWeek };
  }
}
...
console.log(getDayOfWeek(new Date("2023-06-28")));
...
function getDaysInMonth(date) {
  ...
  const lastDay = new Date(newDate - 1);
...
console.log(getDaysInMonth(new Date("2023-06-28")));
console.log(getDaysInMonth(new Date("2023-05-28")));
...
function getFormattedTime(date) {
  ...
  const dateFormat = `${hours.toString().padStart(2, "0")}:${minutes
    .toString()
...
function getAge(birthDate) {
  ...
  let fullYears = todayYear - birthYear;
...
console.log(getAge(new Date("1992-02-29")));
console.log(getAge(new Date("1987-12-24")));
...
console.log(compareDates(new Date("2023-06-28"), new Date("2023-06-28")));
console.log(compareDates(new Date("2023-06-28"), new Date("2023-09-28")));
...
function getDaysDifference(startDate, endDate) {
  ...
  const milisecondsStart = startDate.getTime();
  const milisecondsEnd = endDate.getTime();
  // Різниця в мілісекундах між двома датами.
  // Перетворення різниці в мілісекундах у дні поділивши мілісекунди на (1000 * 60 * 60 * 24).
  const diff = (milisecondsEnd - milisecondsStart) / (1000 * 60 * 60 * 24);
  // Збереження дат для виведення в форматі ISO.
...
console.log(getDaysDifference(new Date("2022-02-24"), new Date("2023-06-28")));
